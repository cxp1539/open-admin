<template>
  <a-card :bordered="false" :style="{minHeight: '592px'}">
    ...
    <div class="table-operator">
      <a-button type="primary" icon="plus" @click="handleCreate">新建用户</a-button>
    </div>
    ...
    <s-table ref="list" rowKey="id" size="default" :columns="columns" :data="loadData">
      ...
      <span slot="username" slot-scope="imageUrl, record">
        <img style="width:42px;height:42px;border-radius:50%;" :src="record.avatar" />
        {{ record.username }}
      </span>
    ...
    <a-modal :title="formFlag == 1 ? '创建用户' : '修改用户'" style="top: 20px;" :width="800" @cancel="cancel" v-model="visible" @ok="handleOk">
      ...
      <a-form ref="ruleForm" :form="form" :labelCol="labelCol" :wrapperCol="wrapperCol">
        ...
        >
          <a-input
            ...
            v-decorator="['password', { rules: [{ validator: checkPassword }] }]"
          />
        </a-form-item>
        ...
        <a-form-item label="用户角色">
          <a-select v-decorator="['roleId', { rules: [{ required: true, message: '请选择用户角色!' }] }]">
            <a-select-option :key="item.id" v-for="item in rolesMap" :value="item.id">{{ item.name }}</a-select-option>
          </a-select>
        </a-form-item>

        <a-form-item label="用户状态">
          <a-select v-decorator="['status', { rules: [{ required: true, message: '请选择用户状态!' }] }]">
            <a-select-option :key="item.id" v-for="item in statusMap" :value="item.id">{{ item.name }}</a-select-option>
          </a-select>
        </a-form-item>
...
import { getRoles, getSysUserList, updateSysUser, createSysUser } from '@/services/system'
